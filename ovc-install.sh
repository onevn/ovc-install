...
# Copyright (c) 2018 OneVN at 1-VN.COM
#
...
# version.
#
# OvnConference is distributed in the hope that it will be useful, but WITHOUT ANY
# WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A
# PARTICULAR PURPOSE. See the GNU Lesser General Public License for more details.
#
# You should have received a copy of the GNU Lesser General Public License along
# with OvnConference; if not, see <http://www.gnu.org/licenses/>.

# OvnConference is an open source conferencing system.  For more information see
#    http://ovc.1-vn.com/.
#
# This ovc-install.sh script automates many of the installation and configuration
# steps at
#    http://docs.ovnconference.org/install/install.html
#
#
#  Examples
#
#  Install OvnConference and configure using server's external IP address
#
#    wget -qO- https://ubuntu.ovnconference.org/ovc-install.sh | bash -s -- -v xenial-220
#
#
#  Install OvnConference and configure using hostname ovc.example.com
#
#    wget -qO- https://ubuntu.ovnconference.org/ovc-install.sh | bash -s -- -v xenial-220 -s ovc.example.com
#
#
#  Install OvnConference with a SSL certificate from Let's Encrypt using e-mail info@example.com:
#
#    wget -qO- https://ubuntu.ovnconference.org/ovc-install.sh | bash -s -- -v xenial-220 -s ovc.example.com -e info@example.com
#
#
#  Install OvnConference with SSL + Greenlight
#
...
Script for installing a OvnConference 2.2 (or later) server in about 15 minutes.

This script also supports installation of a coturn (TURN) server on a separate server.

USAGE:
    wget -qO- https://ubuntu.ovnconference.org/ovc-install.sh | bash -s -- [OPTIONS]
...
OPTIONS (install OvnConference):
  ...
  -m <link_path>         Create a Symbolic link from /var/ovnconference to <link_path> 
...
Sample options for setup a OvnConference server

    -v xenial-220
    -v xenial-220 -s ovc.example.com -e info@example.com
    -v xenial-220 -s ovc.example.com -e info@example.com -g
    -v xenial-220 -s ovc.example.com -e info@example.com -g -c turn.example.com:1234324
...
SUPPORT:
    Community: https://ovnconference.org/support
         Docs: https://github.com/ovnconference/ovc-install
...
main() {
  ...
  PACKAGE_REPOSITORY=ubuntu.ovnconference.org
  ...
  # We're installing OvnConference
  env
  ...
  if [ "$DISTRO" == "xenial" ]; then 
    ...
    need_ppa ovnconference-ubuntu-support-xenial.list ppa:ovnconference/support E95B94BC # Latest version of ffmpeg
  ...
  if [ "$DISTRO" == "bionic" ]; then
    ...
    need_ppa ovnconference-ubuntu-support-bionic.list ppa:ovnconference/support  E95B94BC # Latest version of ffmpeg
...
HERE
  ...
  need_pkg ovnconference
  need_pkg ovc-html5

  if [ -f /usr/share/ovc-web/WEB-INF/classes/ovnconference.properties ]; then
    # 2.2
    SERVLET_DIR=/usr/share/ovc-web
    TURN_XML=$SERVLET_DIR/WEB-INF/classes/spring/turn-stun-servers.xml
  else
    # 2.0
    SERVLET_DIR=/var/lib/tomcat7/webapps/ovnconference
    TURN_XML=$SERVLET_DIR/WEB-INF/spring/turn-stun-servers.xml
  fi

  while [ ! -f $SERVLET_DIR/WEB-INF/classes/ovnconference.properties ]; do sleep 1; echo -n '.'; done
  ...
  if [ ! -z "$API_DEMOS" ]; then
    need_pkg ovc-demo
    while [ ! -f /var/lib/$TOMCAT_USER/webapps/demo/ovc_api_conf.jsp ]; do sleep 1; echo -n '.'; done
  fi

  if [ ! -z "$LINK_PATH" ]; then
    ln -s "$LINK_PATH" "/var/ovnconference"
  fi
...
check_version() {
  if ! echo $1 | egrep -q "xenial|bionic"; then err "This script can only install OvnConference 2.0 (or later)"; fi
  DISTRO=$(echo $1 | sed 's/-.*//g')
  if ! wget -qS --spider "https://$PACKAGE_REPOSITORY/$1/dists/ovnconference-$DISTRO/Release.gpg" > /dev/null 2>&1; then
    err "Unable to locate packages for $1 at $PACKAGE_REPOSITORY."
  fi
  check_root
  need_pkg apt-transport-https
  if ! apt-key list | grep -q "OvnConference apt-get"; then
    wget https://$PACKAGE_REPOSITORY/repo/ovnconference.asc -O- | apt-key add -
  fi

  # Check if were upgrading from 2.0 (the ownership of /etc/ovnconference/nginx/web has changed from ovc-client to ovc-web)
  if [ -f /etc/apt/sources.list.d/ovnconference.list ]; then
    if grep -q xenial-200 /etc/apt/sources.list.d/ovnconference.list; then
      if echo $VERSION | grep -q xenial-220; then
        if dpkg -l | grep -q ovc-client; then
          apt-get purge -y ovc-client
        fi
      fi
    fi
  fi

  echo "deb https://$PACKAGE_REPOSITORY/$VERSION ovnconference-$DISTRO main" > /etc/apt/sources.list.d/ovnconference.list
}
...
check_nat() {
  if [ ! -z "$INTERNAL_IP" ]; then
    ...
    sed -i "s/$INTERNAL_IP:/$IP:/g" /etc/ovnconference/nginx/sip.nginx
...
check_LimitNOFILE() {
  ...
  if [ "$CPU" -gt 36 ]; then
    if [ -f /lib/systemd/system/ovc-web.service ]; then
      # Let's create an override file to increase the number of LimitNOFILE 
      mkdir -p /etc/systemd/system/ovc-web.service.d/
      cat > /etc/systemd/system/ovc-web.service.d/override.conf << HERE
[Service]
LimitNOFILE=
LimitNOFILE=8192
HERE
...
configure_HTML5() {
  ...
  if [ -f /var/www/ovnconference/client/conf/config.xml ]; then
    sed -i 's/offerWebRTC="false"/offerWebRTC="true"/g' /var/www/ovnconference/client/conf/config.xml
  fi

  # Make the HTML5 client default
  sed -i 's/^attendeesJoinViaHTML5Client=.*/attendeesJoinViaHTML5Client=true/'   $SERVLET_DIR/WEB-INF/classes/ovnconference.properties
  sed -i 's/^moderatorsJoinViaHTML5Client=.*/moderatorsJoinViaHTML5Client=true/' $SERVLET_DIR/WEB-INF/classes/ovnconference.properties

  sed -n 's/swfSlidesRequired=true/swfSlidesRequired=false/g'                    $SERVLET_DIR/WEB-INF/classes/ovnconference.properties
}
...
install_greenlight(){
  ...
  SECRET_KEY_BASE=$(docker run --rm ovnconference/greenlight:v2 bundle exec rake secret)

  if [ ! -f ~/greenlight/.env ]; then
    docker run --rm ovnconference/greenlight:v2 cat ./sample.env > ~/greenlight/.env
  fi

  OVNCONFERENCE_URL=$(cat $SERVLET_DIR/WEB-INF/classes/ovnconference.properties | grep -v '#' | sed -n '/^ovnconference.web.serverURL/{s/.*=//;p}')/ovnconference/
  OVNCONFERENCE_SECRET=$(cat $SERVLET_DIR/WEB-INF/classes/ovnconference.properties   | grep -v '#' | grep securitySalt | cut -d= -f2)
  ...
  sed -i "s|.*OVNCONFERENCE_ENDPOINT=.*|OVNCONFERENCE_ENDPOINT=$OVNCONFERENCE_URL|" ~/greenlight/.env
  sed -i "s|.*OVNCONFERENCE_SECRET=.*|OVNCONFERENCE_SECRET=$OVNCONFERENCE_SECRET|"  ~/greenlight/.env

  # need_pkg ovc-webhooks

  if [ ! -f /etc/ovnconference/nginx/greenlight.nginx ]; then
    docker run --rm ovnconference/greenlight:v2 cat ./greenlight.nginx | tee /etc/ovnconference/nginx/greenlight.nginx
    cat > /etc/ovnconference/nginx/greenlight-redirect.nginx << HERE
location = / {
  return 307 /b;
}
HERE
  ...
  if [ ! -f ~/greenlight/docker-compose.yml ]; then
    docker run --rm ovnconference/greenlight:v2 cat ./docker-compose.yml > ~/greenlight/docker-compose.yml
  fi
...
install_ssl() {
  if [ -f /var/www/ovnconference/client/conf/config.xml ]; then
    sed -i 's/tryWebRTCFirst="false"/tryWebRTCFirst="true"/g' /var/www/ovnconference/client/conf/config.xml
  fi

  if ! grep -q $HOST /usr/local/ovnconference/core/scripts/ovnconference.yml; then
    ovc-conf --setip $HOST
  fi
  ...
  if [ ! -f /etc/letsencrypt/live/$HOST/fullchain.pem ]; then
    rm -f /tmp/ovnconference.bak
    if ! grep -q $HOST /etc/nginx/sites-available/ovnconference; then  # make sure we can do the challenge
      cp /etc/nginx/sites-available/ovnconference /tmp/ovnconference.bak
      cat <<HERE > /etc/nginx/sites-available/ovnconference
server {
  listen 80;
  listen [::]:80;
  server_name $HOST;

  access_log  /var/log/nginx/ovnconference.access.log;

  # OvnConference landing page.
  location / {
    root   /var/www/ovnconference-default;
...
HERE
    ...
    if [ -z "$PROVIDED_CERTIFICATE" ]; then
      if ! certbot --email $EMAIL --agree-tos --rsa-key-size 4096 -w /var/www/ovnconference-default/ \
           -d $HOST --deploy-hook "systemctl restart nginx" $LETS_ENCRYPT_OPTIONS certonly; then
        cp /tmp/ovnconference.bak /etc/nginx/sites-available/ovnconference
        systemctl restart nginx
...
server {
  ...
  include /etc/ovnconference/nginx/*.nginx;
...
HERE
  ...
  # Configure rest of OvnConference Configuration for SSL
  ...
  sed -i 's/http:/https:/g' /etc/ovnconference/nginx/sip.nginx
  sed -i 's/5066/7443/g'    /etc/ovnconference/nginx/sip.nginx

  sed -i 's/ovnconference.web.serverURL=http:/ovnconference.web.serverURL=https:/g' $SERVLET_DIR/WEB-INF/classes/ovnconference.properties

  if [ -f /var/www/ovnconference/client/conf/config.xml ]; then
    sed -i 's|http://|https://|g' /var/www/ovnconference/client/conf/config.xml
  ...
  yq w -i /usr/local/ovnconference/core/scripts/ovnconference.yml playback_protocol https
  chmod 644 /usr/local/ovnconference/core/scripts/ovnconference.yml 

  if [ -f /var/lib/$TOMCAT_USER/webapps/demo/ovc_api_conf.jsp ]; then
    sed -i 's/String OvnConferenceURL = "http:/String OvnConferenceURL = "https:/g' /var/lib/$TOMCAT_USER/webapps/demo/ovc_api_conf.jsp
  fi
  ...
  if [ -f  /usr/share/meteor/bundle/programs/server/assets/app/config/settings-production.json ]; then
    sed -i "s|\"wsUrl.*|\"wsUrl\": \"wss://$HOST/ovc-webrtc-sfu\",|g" \
      /usr/share/meteor/bundle/programs/server/assets/app/config/settings-production.json
  fi

  TARGET=/usr/local/ovnconference/ovc-webrtc-sfu/config/default.yml
  if [ -f $TARGET ]; then
    ...
    chown ovnconference:ovnconference $TARGET
    chmod 644 $TARGET
  fi
}
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="stun0" class="org.ovnconference.web.services.turn.StunServer">
        <constructor-arg index="0" value="stun:$COTURN_HOST"/>
    </bean>


    <bean id="turn0" class="org.ovnconference.web.services.turn.TurnServer">
        <constructor-arg index="0" value="$COTURN_SECRET"/>
        <constructor-arg index="1" value="turns:$COTURN_HOST:443?transport=tcp"/>
        <constructor-arg index="2" value="86400"/>
    </bean>
    
    <bean id="turn1" class="org.ovnconference.web.services.turn.TurnServer">
        <constructor-arg index="0" value="$COTURN_SECRET"/>
    ...
    <bean id="stunTurnService"
            class="org.ovnconference.web.services.turn.StunTurnService">
...
# Example coturn configuration for OvnConference
...
# limited credentials. OvnConference will generate credentials in this
# format. Note that the static-auth-secret value specified here must match
# the configuration in OvnConference's turn-stun-servers.xml
...
#
# This TURN server is ready.  To configure your OvnConference server to use this TURN server, 
# add the option
#
#  -c $COTURN_HOST:$COTURN_SECRET
#
# the the ovc-install.sh command.
#
HERE
}
